...
from myth_hash import hash_name, load_attributes, load_character_nouns
...
class TestMythHashGenerator(unittest.TestCase):
    ...
    def test_hash_name(self):
        characters = load_character_nouns()
        physical_attributes = load_attributes(
            Path(__file__).parent / "physical_attributes.json"
        )
        personality_attributes = load_attributes(
            Path(__file__).parent / "personality_attributes.json"
        )

        result = hash_name(
            "teststring", physical_attributes, personality_attributes, characters, "en"
        )
...
class TestNameUniqueness(unittest.TestCase):
    ...
    def test_name_uniqueness(self):
        ...
        expected_collisions = calculate_expected_collisions(
            len(physical_attributes),
